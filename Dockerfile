...
HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
  CMD sh -c "node -e \"const http = require('http'); const port = process.env.PORT || 8080; const req = http.get('http://127.0.0.1:' + port + '/health', (res) => { process.exit(res.statusCode === 200 ? 0 : 1); }); req.on('error', () => process.exit(1)); req.setTimeout(5000, () => process.exit(1));\""
