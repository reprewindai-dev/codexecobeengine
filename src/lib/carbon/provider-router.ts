...
import { FingardCore } from '../fingard/fingard-core'
...
export class ProviderRouter {
  private fingard: FingardCore | null

  constructor(options?: { useFingard?: boolean }) {
    this.fingard = options?.useFingard === false ? null : new FingardCore()
  }

  /**
   * Produce a routing signal using Fingard control layer:
   *
   * 1. Fingard region-specific signal selection
   * 2. Confidence scoring and validation
   * 3. Hard fail protection with fallback
   * 4. Legacy provider logic as fallback
   */
  async getRoutingSignal(region: string, timestamp: Date): Promise<RoutingSignal> {
    if (!this.fingard) {
      return await this.getLegacyRoutingSignal(region, timestamp)
    }

    try {
      // ── TIER 0: Fingard Control Layer ───────────────────────────────
      
      // Use Fingard for signal selection, validation, and fallback protection
      const fingardResult = await this.fingard.getRoutingSignal(region, timestamp)
      const fingardSignal = fingardResult.signal

      // Convert FingardSignal back to RoutingSignal format
      const routingSignal: RoutingSignal = {
        carbonIntensity: fingardSignal.gco2,
        source: this.mapFingardSourceToRoutingSource(fingardSignal.source),
        isForecast: (fingardSignal.metadata?.isForecast as boolean) || false,
        confidence: fingardSignal.confidence / 100, // Convert back to 0-1 scale
        provenance: {
          sourceUsed: `${fingardSignal.source}_FINGARD`,
          contributingSources: [fingardSignal.source],
          referenceTime: timestamp.toISOString(),
          fetchedAt: new Date().toISOString(),
          fallbackUsed: fingardResult.fallbackReason !== 'NO_FALLBACK',
          disagreementFlag: false,
          disagreementPct: 0,
          validationNotes: fingardResult.fallbackReason === 'NO_FALLBACK' 
            ? `Fingard selected: ${fingardSignal.source} (${fingardSignal.confidence.toFixed(1)}% confidence)`
            : `Fingard fallback: ${fingardResult.fallbackReason}`
        }
      }

      return routingSignal
    } catch (error) {
      // ── FALLBACK: Legacy Provider Logic ─────────────────────────────
      
      console.warn(`Fingard failed for ${region}, falling back to legacy logic:`, error)
      return await this.getLegacyRoutingSignal(region, timestamp)
    }
  }

  /**
   * Map Fingard source back to RoutingSignal source format
   */
  private mapFingardSourceToRoutingSource(source: string): RoutingSignal['source'] {
    const sourceMap: Record<string, RoutingSignal['source']> = {
      'WATTTIME': 'watttime',
      'EIA': 'gridstatus_fuel_mix',
      'GB': 'gb_carbon_intensity',
      'DK': 'dk_carbon',
      'FI': 'fi_carbon',
      'EMBER': 'ember',
      'STATIC': 'fallback'
    ...
    return sourceMap[source] || 'fallback'
  }

  /**
   * Legacy routing signal logic (original implementation)
   * Used as fallback when Fingard fails
   */
  private async getLegacyRoutingSignal(region: string, timestamp: Date): Promise<RoutingSignal> {
